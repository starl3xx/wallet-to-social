...
- docs/DOCS-SITE.md corrected: inbound mail is Google Workspace now (MX
  `smtp.google.com`), not Cloudflare Email Routing.
...
- **A lookup that has been charged is never told to submit again.** The
  finished results are saved before the charge, so a lookup stopped after it
  is finished from them on the next attempt, with no second pass over the
  sources and no second charge. Neither the attempt limit nor an error after
  the charge can turn it into "Submit the list again", which would have left
  the charge standing and billed the resubmission as well. In the one case
  where a charged lookup cannot be finished (its results were never saved,
  for example a rerun that keeps failing), it stops after five attempts with
  "Processing stopped after this lookup was charged. Contact
  help@walletlink.social for a rerun or a refund."
- **A lookup is saved to your history once.** A lookup finished twice (stopped
  ...
  once. If a later pass decides a different match limit than the first (the
...
- Operator: `scripts/migrate-job-lease.ts` adds `lookup_jobs.leased_until`,
  ...
  lease, and a retry or rerun detaches the previous saved lookup. The
...
- **A database failure during a refresh answers 503 `temporarily_unavailable`
  ...
  start consent over. The failure is logged, so one that repeats is visible.
- **A refresh is bound to its client and resource** (OAuth 2.1 section 4.3.1,
  ...
  revoking; an absent or empty one is still accepted. A request naming another
  `resource`, in any of its values, answers `invalid_target`; a grant made for
  another server answers `invalid_grant`, so a client starts over instead of
  looping.
- **The token endpoint refuses a repeated parameter other than `resource`**
  (OAuth 2.1 section 3.2), and treats a parameter sent without a value as
  omitted. The code exchange now compares `resource` by the rule authorization
  and refresh use, where it compared exact strings: a trailing slash on one
  side no longer strands a connection halfway.
...
- A resource with a fragment is refused, an empty one included (RFC 8707
  section 2).
- Tests: 52 new invariants and 34 new guard mutations.
