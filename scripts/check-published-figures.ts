  ...
  },
  {
    ...
    actual: async () => {
      ...
      const attested = await one(sql`
        ...
        WHERE twitter_handle IS NOT NULL
          AND sources && ARRAY['farcaster_sweep','neynar','ens_onchain','ens','ethos','eas','clanker','manual','debank_tweet','sybil_list','snapshot_profile','lens_profile','opensea_profile','ud_profile','ud_twitter_validation','basename_record','owner_attested']`);
