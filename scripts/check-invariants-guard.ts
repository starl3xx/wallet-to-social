...
const MUTATIONS: Mutation[] = [
  ...
  {
    name: 'the privacy policy restates the backup period as a digit in its table',
    file: 'app/privacy/page.tsx',
    from: 'suppression list: ${BACKUP_RETENTION_DAYS} days from each nightly copy`,',
    to: 'suppression list: 90 days from each nightly copy`,',
  },
  {
    name: 'the privacy policy restates the backup period as a digit for GitHub',
    file: 'app/privacy/page.tsx',
    from: '              holds, and each is kept for {BACKUP_RETENTION_DAYS} days.',
    to: '              holds, and each is kept for 90 days.',
  },
  {
    name: 'the privacy policy restates the withdrawal email period as a digit',
    file: 'app/privacy/page.tsx',
    from: "            from a backup; we delete that email after {BACKUP_RETENTION_DAYS}{' '}\n            days,",
    to: '            from a backup; we delete that email after 90\n            days,',
  },
  {
    name: 'the privacy policy goes back to deleting the removal thread once the removal is done',
    file: 'app/privacy/page.tsx',
    from: '            together. We keep the email thread, your message and our replies,\n            for {BACKUP_RETENTION_DAYS} days after the removal is done, so that\n            we can apply the removal again if we ever restore our database from\n            a backup, and then delete it.',
    to: '            together, and once the removal is done we delete the email thread,\n            your message and our replies.',
  },
  {
    name: 'the privacy policy keeps the withdrawal email with no stated end',
    file: 'app/privacy/page.tsx',
    from: "; we delete that email after {BACKUP_RETENTION_DAYS}{' '}\n            days, when no backup older than it is left.",
    to: '.',
  },
  {
    name: 'the privacy policy drops the retention row for support-inbox emails',
    file: 'app/privacy/page.tsx',
  ...
  {
    name: 'the backup constant the privacy policy reads drifts from the workflow',
    file: 'lib/backup-retention.ts',
    from: 'export const BACKUP_RETENTION_DAYS = 90;',
    to: 'export const BACKUP_RETENTION_DAYS = 30;',
  },
  {
    name: 'the backup workflow keeps each copy longer than the privacy policy says',
    file: '.github/workflows/db-backup.yml',
    from: '          retention-days: 90',
  ...
  },
  {
    ...
    file: 'lib/ops-alerts.ts',
    from: '    claimed = await claimAlerts(db, family, conditions, payloads);',
    to: '    await claimAlerts(db, family, conditions, payloads);\n    claimed = conditions;',
  },
  {
  ...
  },
  {
    ...
    file: 'lib/ops-alerts.ts',
    from: '    await releaseAlerts(db, family, claimed);',
    to: '    void claimed;',
  },
  {
  ...
  // --- STA-50: the withdrawal email trail (2026-09-26) -------------------
  {
    name: 'STA-50 the withdrawal email goes out before the erase, so a failed erase still emails',
    file: 'app/api/claim/withdraw/route.ts',
    from: "  const erased = await eraseIdentifier(db, 'wallet', wallet);\n\n  /**\n   * The email trail (STA-50), only now that the withdrawal is committed: both\n   * statements above autocommit, and the erase throws on any failed step, so\n   * reaching this line means the whole withdrawal is in the database. It\n   * names the rows written to the suppression list and the claims this\n   * person withdrew, for the operator to re-apply after a restore. Awaited so\n   * the record is written before the function can be frozen; it never\n   * throws, and a send that fails is left for the daily cleanup.\n   */\n  await alertClaimWithdrawal(\n    {\n      suppressed,\n      claimIds: found.rows.map((r) => r.id),\n      withdrawnAt: new Date().toISOString(),\n    },\n    db\n  );\n",
    to: "  await alertClaimWithdrawal(\n    {\n      suppressed,\n      claimIds: found.rows.map((r) => r.id),\n      withdrawnAt: new Date().toISOString(),\n    },\n    db\n  );\n  const erased = await eraseIdentifier(db, 'wallet', wallet);\n\n  /**\n   * The email trail (STA-50), only now that the withdrawal is committed: both\n   * statements above autocommit, and the erase throws on any failed step, so\n   * reaching this line means the whole withdrawal is in the database. It\n   * names the rows written to the suppression list and the claims this\n   * person withdrew, for the operator to re-apply after a restore. Awaited so\n   * the record is written before the function can be frozen; it never\n   * throws, and a send that fails is left for the daily cleanup.\n   */\n",
  },
  {
    name: 'STA-50 the withdrawal sends no email',
    file: 'app/api/claim/withdraw/route.ts',
    from: '  await alertClaimWithdrawal(\n    {\n      suppressed,\n      claimIds: found.rows.map((r) => r.id),\n      withdrawnAt: new Date().toISOString(),\n    },\n    db\n  );\n',
    to: '',
  },
  {
    name: 'STA-50 the withdrawal email is fired and forgotten',
    file: 'app/api/claim/withdraw/route.ts',
    from: '  await alertClaimWithdrawal(\n',
    to: '  void alertClaimWithdrawal(\n',
  },
  {
    name: 'STA-50 the email names a list other than the one the route suppressed',
    file: 'app/api/claim/withdraw/route.ts',
    from: '      suppressed,\n      claimIds:',
    to: '      suppressed: [],\n      claimIds:',
  },
  {
    name: 'STA-50 the route gives the email no claim reference',
    file: 'app/api/claim/withdraw/route.ts',
    from: '      claimIds: found.rows.map((r) => r.id),',
    to: '      claimIds: [],',
  },
  {
    name: 'STA-50 the claim check counts claims from any account',
    file: 'app/api/claim/withdraw/route.ts',
    from: '    WHERE user_id = ${session.user.id}\n      AND wallet = ${wallet}',
    to: '    WHERE wallet = ${wallet}',
  },
  {
    name: 'STA-50 the email body drops the suppressed identifiers',
    file: 'lib/removal-alerts.ts',
    from: '      ...r.suppressed.map((t) => `Suppressed: ${t.kind} ${t.identifier}`),\n',
    to: '',
  },
  {
    name: 'STA-50 the email body drops the claim reference',
    file: 'lib/removal-alerts.ts',
    from: "      `Claim reference: ${r.claimIds.join(', ')}`,\n",
    to: '',
  },
  {
    name: 'STA-50 the email body drops the time',
    file: 'lib/removal-alerts.ts',
    from: '      `Withdrawn at: ${r.withdrawnAt}`,\n',
    to: '',
  },
  {
    name: 'STA-50 the subject names the wallet',
    file: 'lib/removal-alerts.ts',
    from: '    subject: WITHDRAWAL_SUBJECT,',
    to: '    subject: `${WITHDRAWAL_SUBJECT} ${r.suppressed[0]?.identifier}`,',
  },
  {
    name: 'STA-50 a sent withdrawal keeps the wallet in the database',
    file: 'lib/removal-alerts.ts',
    from: '  dropPayloadWhenSent: true,',
    to: '  dropPayloadWhenSent: false,',
  },
  {
    name: 'STA-50 a sent record keeps its payload whatever the family says',
    file: 'lib/ops-alerts.ts',
    from: "${family.dropPayloadWhenSent ? sql` - 'payload'` : sql``}",
    to: '',
  },
  {
    name: 'STA-50 a withdrawal email that fails throws into the withdrawal response',
    file: 'lib/removal-alerts.ts',
    from: "    console.error(\n      redact(format('[removal] withdrawal email not recorded:', error))\n    );\n    return 'failed';",
    to: '    throw error;',
  },
  {
    name: 'STA-50 the withdrawal log line skips redact',
    file: 'lib/removal-alerts.ts',
    from: "      redact(format('[removal] withdrawal email not recorded:', error))",
    to: "      format('[removal] withdrawal email not recorded:', error)",
  },
  {
    name: 'STA-50 the shared alert log line skips redact',
    file: 'lib/ops-alerts.ts',
    from: '    redact(\n      error === undefined',
    to: '    String(\n      error === undefined',
  },
  {
    name: 'STA-50 the daily cleanup never resends a failed withdrawal email',
    file: 'app/api/cron/cleanup/route.ts',
    from: '  const removalAlertRecords = await sendUnsentRemovalRecords(db);',
    to: '  const removalAlertRecords = null;',
  },
  {
    name: 'STA-50 the runbook names a subject the email does not carry',
    file: 'docs/OPERATIONS.md',
    from: '  `[walletlink] Removal: claim withdrawn on /claim`, which names nobody.',
    to: '  `[walletlink] Withdrawal on /claim`, which names nobody.',
  },
  {
    name: 'STA-50 the runbook re-applies a withdrawal on the wrong lane',
    file: 'docs/OPERATIONS.md',
  ...
  // --- STA-50 decision: help@ keeps removal emails 90 days (2026-09-26) ---
  {
    name: 'STA-50 the help@ script moves threads to Trash instead of deleting them',
    file: 'scripts/ops/help-inbox-removal-retention.gs',
    from: "    Gmail.Users.Threads.remove('me', id);",
    to: "    Gmail.Users.Threads.trash('me', id);",
  },
  {
    name: 'STA-50 the help@ script moves threads to Trash through GmailApp',
    file: 'scripts/ops/help-inbox-removal-retention.gs',
    from: "    Gmail.Users.Threads.remove('me', id);",
    to: '    GmailApp.getThreadById(id).moveToTrash();',
  },
  {
    name: 'STA-50 the help@ script keeps removal emails 30 days, not the backups’ 90',
    file: 'scripts/ops/help-inbox-removal-retention.gs',
    from: 'const RETENTION_DAYS = 90;',
    to: 'const RETENTION_DAYS = 30;',
  },
  {
    name: 'STA-50 the help@ script cutoff is fixed in days, whatever the threshold says',
    file: 'scripts/ops/help-inbox-removal-retention.gs',
    from: '  const cutoff = Date.now() - RETENTION_DAYS * 24 * 60 * 60 * 1000;',
    to: '  const cutoff = Date.now() - 30 * 24 * 60 * 60 * 1000;',
  },
  {
    name: 'STA-50 the help@ script deletes on Gmail’s older_than alone, not by the last message',
    file: 'scripts/ops/help-inbox-removal-retention.gs',
    from: '      if (thread.getLastMessageDate().getTime() < cutoff) {\n        due.push(thread.getId());\n      }',
    to: '      due.push(thread.getId());',
  },
  {
    name: 'STA-50 the help@ script reads only the first page of results',
    file: 'scripts/ops/help-inbox-removal-retention.gs',
    from: '    if (threads.length < PAGE_SIZE) break;',
    to: '    break;',
  },
  {
    name: 'STA-50 the help@ script deletes every labeled thread, not only removal ones',
    file: 'scripts/ops/help-inbox-removal-retention.gs',
    from: "const LABEL = 'walletlink-removals';",
    to: "const LABEL = 'inbox';",
  },
  {
    name: 'STA-50 the help@ script logs the subject of each thread it deletes',
    file: 'scripts/ops/help-inbox-removal-retention.gs',
    from: '        due.push(thread.getId());\n',
    to: '        due.push(thread.getId());\n        console.log(thread.getFirstMessageSubject());\n',
  },
  {
    name: 'STA-50 the help@ filter step names a subject the withdrawal email does not carry',
    file: 'scripts/ops/help-inbox-removal-retention.gs',
    from: ' *    "[walletlink] Removal: claim withdrawn on /claim" (in quotes) and the',
    to: ' *    "[walletlink] Withdrawal on /claim" (in quotes) and the',
  },
  {
    name: 'STA-50 the backup constant drifts from the workflow',
    file: 'lib/backup-retention.ts',
    from: 'export const BACKUP_RETENTION_DAYS = 90;',
    to: 'export const BACKUP_RETENTION_DAYS = 30;',
  },
  {
    name: 'STA-50 the backups are kept longer than every text and the script say',
    file: '.github/workflows/db-backup.yml',
    from: '          retention-days: 90',
    to: '          retention-days: 120',
  },
  {
    name: 'STA-50 the runbook drops the manual label step for emailed requests',
    file: 'docs/OPERATIONS.md',
    from: '3. Label each emailed removal request by hand when its removal is done\n   (the reply policy above). No filter can find these, and a request\n   thread without the label is kept.\n',
    to: '',
  },
  {
    name: 'STA-50 the runbook goes back to deleting a request thread at once',
    file: 'docs/OPERATIONS.md',
    from: '**Then label the thread `walletlink-removals`.** After the reply confirming\nexecution, apply the Gmail label `walletlink-removals` to the request\nthread by hand.',
    to: '**Then delete the thread.** After the reply confirming\nexecution, delete the request thread.',
  },
  {
    name: 'STA-50 the reply script tells the requester a different period',
    file: 'docs/OPERATIONS.md',
    from: '> ones you name. We keep this email thread for 90 days after the removal is',
    to: '> ones you name. We keep this email thread for 30 days after the removal is',
  },
  {
    name: 'STA-50 the runbook states a retention the script does not use',
    file: 'docs/OPERATIONS.md',
    from: '**The help@ retention: removal emails are kept 90 days, then deleted**',
    to: '**The help@ retention: removal emails are kept 60 days, then deleted**',
  },
  {
    name: 'STA-50 the changelog states a retention the script does not use',
    file: 'CHANGELOG.md',
    from: '- **Removal emails in help@ are kept 90 days, then deleted** (decided',
    to: '- **Removal emails in help@ are kept 30 days, then deleted** (decided',
  },
  {
    name: 'STA-50 the overview says the deletion goes through Trash',
