...
[MCP server](/mcp-server). When the pack runs dry, the metered endpoints answer
`402 NO_CREDITS`, but [`/v1/usage`](/api-reference/usage) and
[`/v1/stats`](/api-reference/stats) keep answering at zero balance: a drained
key can always read its own meter, and that zero reading is the signal to buy
again.
