...
The key resolves addresses: the single, batch and job lookups, the estimate,
and the free statistics and usage reads. The reverse lookups, from an X handle
or a Farcaster username to the wallets behind it, need an account and answer
this key with `403` `ACCOUNT_REQUIRED`: that direction can find a person, so
somebody has to answer for the search.
...
The same `402` states the [terms of service](https://walletlink.social/terms)
the purchase is made under, and paying the challenge accepts them. Nothing
extra is sent to do so: the request that pays is the same request it was
before. The terms appear in three places, because x402 clients read different
parts of the response:

- The resource description inside `PAYMENT-REQUIRED` ends with the terms URL
  and version, so a client that pays automatically has read them.
- The JSON body carries them as fields:

  ```json
  {
    "error": "Payment required: 12 match credits for $1.00 in USDC on Base.",
    "code": "PAYMENT_REQUIRED",
    "terms": {
      "url": "https://walletlink.social/terms",
      "version": "2026-09-25",
      "note": "Paying accepts the terms of service at https://walletlink.social/terms, version 2026-09-25."
    }
  }
  ```

- A `Link: <https://walletlink.social/terms>; rel="terms-of-service"` header
  ([RFC 6903](https://www.rfc-editor.org/rfc/rfc6903)).

The version is the date the terms were last updated. The credits your payment
buys record the version in force when the payment settled, and the time.
...
## When a purchase is refused before payment

The paying wallet is screened against OFAC’s SDN list as soon as the paid
request arrives, and the payer that is screened is the payer that pays: a
payment payload must be an EIP-3009 authorization and its signature, from an
EVM address, and nothing else, or it is refused with `400` `INVALID_PAYMENT`.
Purchases are not available from a country or region under comprehensive US
sanctions. These refusals all happen **before the payment is settled**, so
nothing is charged:

| Status | Code                    | Meaning                                                                                                                           |
| ------ | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `403`  | `REGION_RESTRICTED`     | Purchases are not available where the request comes from. Card checkout gives the same answer.                                    |
| `403`  | `SANCTIONED_PAYER`      | This payment cannot be accepted. The response says nothing more.                                                                  |
| `503`  | `SCREENING_UNAVAILABLE` | The screen could not run, so the sale was paused rather than made unchecked. Retry after the seconds in the `Retry-After` header. |

The `503` can also come after the payment is verified, when the screening
cannot be recorded; it still comes before settlement.

An account whose paying wallet is found on OFAC’s SDN list after it bought is
suspended: its keys stop working and its credits are held. A USDC buy into it,
a top-up with its key, a card checkout while signed in to it, and
[key recovery](#lost-your-key) all answer `403` `SANCTIONED_PAYER`.
