...
<Warning>
  `shown_once` is literal. The key is stored only as a hash and there is no
  email on the account to recover it through. Persist it before the process
  exits.
</Warning>
...
not against the transaction hash, so replaying a payment returns the credits it
already bought rather than charging again. A response with
`"newly_granted": false` means exactly that: the key is fresh, the credits are
