...
Where both accounts still reach someone, we keep the handle the record already
had and settle nothing. Measured on real cases, when both are live the handle we
hold usually belongs to somebody who does not claim the wallet, so picking a
winner there would be a guess.

Where the handle we hold reaches nobody and the other one is live, and both of
those were checked recently, we follow the rename: the live handle becomes the
record, and the handle it replaced is kept so the change can be explained rather
than appearing as a correction out of nowhere. That is the whole of what we
