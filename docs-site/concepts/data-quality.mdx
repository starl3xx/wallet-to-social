...
<Warning>
  ...
  everyday sense. The flag is true for handles attested by an onchain record, a
  manual review, or an attested-social sign-in; the majority Farcaster-attested
  handles carry false. A handle that came from an X account verified on
...
can decide it three ways. Where the handle we hold reaches nobody and the other
...
person, and we follow the owner. And where the challenger carries the owner’s
permanent account id, confirmed against the account it resolves to today, while
the handle we hold is a bare string no id ever confirmed, the anchored statement
outweighs the unanchored one.

In every settled case the replaced handle is kept, so the change can be
explained rather than appearing as a correction out of nowhere. Where both
accounts are live and both carry consistent ids, two owner statements disagree
and neither outweighs the other: we keep the handle the record already had and
settle nothing, because picking a winner there would be a guess.
