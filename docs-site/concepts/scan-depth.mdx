...
A fast scan’s batches read only the index, so each takes seconds and a fast scan of 10,000 addresses finishes in its first pass. A deep scan waits on live sources in every batch, so a long one can take several passes, and its time follows the deep figure above.
