...
<CardGroup cols={2}>
  <Card title="Fast" icon="bolt">
    Answers from our index only. No live request goes out, so a list of any
    size comes back in seconds.
  </Card>
  <Card title="Deep scan" icon="binoculars">
    The index plus every live source, including onchain ENS records. Slower,
    and finds the most.
  </Card>
</CardGroup>
...
A fast scan reads two things: the identity index, and the short-term cache in front of it. That is the whole pipeline. Nothing is resolved live, nothing is written back, and the wall clock is dominated by the round trip rather than by how many addresses you sent.

A deep scan reads those first, then resolves whatever is left against every live source available to your plan, including onchain ENS text records. It writes what it learns back, so the next lookup over the same addresses is faster for everyone.

<Note>
Onchain ENS reads are a paid feature. A free account choosing a deep scan gets
every other source rather than an error, so the choice is always available.
</Note>
...
A fast scan does not update the index, and this is deliberate rather than an optimisation. Writing back would stamp records as freshly checked when nothing checked them, and a later deep scan would trust that stamp and skip the very sources meant to confirm them. One fast scan would suppress the next real one.
