...
Discovery is limited per IP address, at 120 requests an hour. That covers
`initialize`, `tools/list` and the other handshake methods, which answer without
reaching the API and so are not metered against any key. Tool calls are not
subject to it: they are metered against your key instead.
