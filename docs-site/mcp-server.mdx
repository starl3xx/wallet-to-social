...
closes it. Repeated OAuth parameters are refused, except `resource`, which may
repeat as long as every value is this server.

A refresh must come from the client the connection was made with: send the same
`client_id`, or none. A `resource` on a refresh must name this server. If the
