...
`temporarily_unavailable` and `Retry-After`, for a code exchange as for a
refresh. Retry with the same code (it lasts a minute) or the same refresh token:
a failure leaves both unspent. The one exception is a request that completed
