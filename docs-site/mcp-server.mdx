---
title: 'MCP server'
description: 'Eight tools that let an agent resolve wallets to social identities, over the same API and the same balance.'
---
...
A metadata document's URL, the `client_id`, must be `https` with a path, carry
no credentials and no fragment, name a host rather than an IP address, and be
written the way a URL parser prints it: a lowercase host, no default port, no
`.` or `..` segments. Its host must resolve only to public addresses. The
document is fetched within 5 seconds, uncompressed, with no redirect followed,
and must be at most 64 KB and name its own URL as `client_id`; it is cached for
a day. Registration keeps `authorization_code` and `refresh_token`, drops any
other grant type it is sent, and returns the grant types it registered. A
registration without `authorization_code` is refused.

The consent screen always says where the reply goes: the host of the redirect
in the request you are approving. When that host is this computer, it says so
and asks you to approve only a connection you started yourself. That warning
is normal for Claude Code and MCP Inspector, which receive their reply on a
local port.

**For client developers.** A `redirect_uri` must match one the client declared,
character for character, except that a loopback redirect (`localhost`,
`127.0.0.1` or `[::1]`, over `http`) may use any port. An error, or a person
declining, is sent back to the client's callback only when that callback is on
loopback or on claude.ai. For any other callback the answer is shown on the
consent page instead, and the client should stop waiting when the person
closes it. Repeated OAuth parameters are refused, at the consent page and the
token endpoint alike, except `resource`, which may repeat as long as every value
is this server.

A refresh token rotates on every use; present each one once. While rotations
keep coming within 30 seconds of each other, as they do when several calls
from one client refresh at once, a token already rotated out answers `503`
`temporarily_unavailable` with `"token_rotated": true` and no tokens. Do not
send that refresh token again: use the tokens the other request received, and
the connection carries on. After that window, presenting the token a rotation
replaced ends the connection, and an older one is unknown.

A refresh must come from the client the connection was made with: send the same
`client_id`, or none. Every `resource` on a refresh must name this server. If the
token service is briefly unavailable it answers `503` with
`temporarily_unavailable` and `Retry-After`, without `token_rotated`, for a code
exchange as for a refresh. Retry with the same code (it lasts a minute) or the
same refresh token: a failure leaves both unspent. The one exception is a request that completed
and lost only its reply; its retry is refused, and the client connects again.
...
**The tools that spend credits ask first.** `walletlink_resolve_wallets`, both
reverse lookups and `walletlink_submit_job` are marked as not read-only, so a
client such as Claude asks you before each call. The four free tools are
marked read-only and can run without asking. None of the tools deletes
anything, and all but `walletlink_submit_job` answer from walletlink's own
index.
...
with no credits. That is the same behavior the REST API has. The app is
different: a lookup there that finds more matches than the balance covers opens
the covered ones and locks the rest, past a small margin. See
[when a lookup finds more than your balance covers](/app/lookups#when-a-lookup-finds-more-than-your-balance-covers).
...
strongest evidence, not each identity separately. See
[the verified flags](/concepts/data-quality#the-verified-flags).

`x.self_declared` is `true` when the handle rests only on text the wallet owner
typed into their own record, such as an ENS text record: the owner claims the
handle, and the X account never confirmed it. It is omitted otherwise, and
absent is not false. See
[self-declared handles](/concepts/data-quality#self-declared-handles).
...
resolving anything. On an account that is never debited it answers
`unmetered: true` with `matches_available` null: there is no balance to report,
which is not the same as a zero one.
...
credential is refused with HTTP 401 and a `WWW-Authenticate` header rather than
a tool error, and so is any request at all, `initialize` and `tools/list`
included, that carries an OAuth access token which has expired or been revoked.
A client reads that as a signal to refresh its token or ask you to sign in, and
then retries the same request; a tool error would be handed to the model as
text and the turn would move on with nothing fixed. A request with no
credential that is not a tool call still answers, so a client can connect and
...
OAuth token requests, the exchange that completes sign-in and every refresh
after it, are limited per connection, not per IP address.

Tool calls are limited per key, on the same three windows as the REST API. See
[rate limits](/api-reference/rate-limits).

Everything except a tool call answers without reaching the API, so it is not
metered against any key, and it is limited per hour instead. That covers
`initialize`, `tools/list` and every other method:

- **With a credential that works**, an API key or an OAuth connection, the
  limit is 600 requests an hour per account. Every key and connection on the
  account shares it.
- **Without one**, including a key that does not validate, the limit is 120
  requests an hour per IP address.

The split matters for hosted clients. Claude on the web, desktop and mobile
calls MCP servers from Anthropic’s shared addresses, so a per-address limit
would be shared by everybody using it at once; a connected account is counted
on its own. Each refusal is a 429 with `Retry-After` whose message says which
limit was reached and what to do about it.

Tool calls are not subject to either. They are metered against your key
instead, on the three windows above.
