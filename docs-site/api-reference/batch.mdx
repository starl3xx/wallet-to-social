...
<ResponseField name="meta" type="object">
  <Expandable title="properties">
    ...
    <ResponseField name="matched" type="number">
      How many resolved to an X handle or a Farcaster account. On a fresh
      request this is the number billed, and it is never more than `found`. For
      a replay, see [retries](#retries-and-the-idempotency-key).
    </ResponseField>
...
Farcaster account**, after deduplication. On a fresh request, `meta.matched` is
exactly that count; for a replay, see
[retries](#retries-and-the-idempotency-key).
...
- The dedup is scoped to your API key and pinned to the exact request body, and
  lasts 24 hours. A replay returns the original status and the original body,
  less any removals (below), with an `Idempotency-Replayed: true` header, bills
  no matches, and resolves nothing.
- A replay honors removals made since the original request. An address or
  account whose owner asked to be removed in the meantime is left out of the
  replay exactly as a fresh request would leave it out, and `found`,
  `not_found` and `matched` count what the replay returns, so `matched` can be
  below what the original request billed. The replay itself bills nothing, and
  your usage and balance remain the billing record.
