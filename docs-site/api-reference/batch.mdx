...
Resolves many addresses in one request. Costs **1 match credit per address that resolves**, after duplicates are removed. Addresses that match nothing cost nothing.
...
<ParamField body="wallets" type="string[]" required>
  Addresses to resolve. Up to 50, or 200 on a legacy Unlimited account. The whole body is capped at 1 MB.
</ParamField>
...
Match credits are charged on the addresses that **resolve**, after
deduplication. Submitting `[A, B, A]` where A resolves and B does not costs 1
credit and returns 2 records. Deduplicating still helps, because duplicates
