...
Batch answers from the index, which is why it returns in milliseconds. For a list larger than a batch, or when the misses are worth re-checking against live sources, submit an [async job](/api-reference/jobs) instead: same billing rule, no 50-address ceiling, and live resolution on wallets the index has not checked.
...
<ParamField body="wallets" type="string[]" required>
  Addresses to resolve. Up to 50. The whole body is capped at 1 MB.
</ParamField>
