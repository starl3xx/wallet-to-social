...
|                            | **Any pack** |
| -------------------------- | ------------ |
| Plan name in API responses | `Developer`  |
...
metered call made with no credits left returns `402` with code `NO_CREDITS`.
The two free endpoints, [`/v1/stats`](/api-reference/stats) and
[`/v1/usage`](/api-reference/usage), keep answering at zero balance.

<Note>
  Responses from [`/v1/usage`](/api-reference/usage) report the internal plan
  name rather than the pack you purchased. Every pack reports `Developer`.
</Note>
...
`NO_CREDITS`. The two zero-credit endpoints answer at any balance, so a drained
key can always read its own meter.
