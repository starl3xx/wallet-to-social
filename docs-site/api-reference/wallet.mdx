...
<ResponseField name="data" type="object | null">
  ...
  <Expandable title="properties">
    ...
    <ResponseField name="twitter" type="object">
      `handle`, `url`, `verified`, `self_declared` where the handle rests only on text the wallet owner typed into their own record, and, once we have checked, `reachable`, `reachability` and `reachability_checked_at`. See [the verified flags](/concepts/data-quality#the-verified-flags), which are narrower than they look, [self-declared handles](/concepts/data-quality#self-declared-handles), and [when a handle stops being true](/concepts/data-quality#when-a-handle-stops-being-true).
