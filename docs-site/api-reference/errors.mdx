...
| `INVALID_REQUEST`            | 400    | Body is missing, not valid JSON, `wallets` is absent or empty, or an `Idempotency-Key` header is over 200 characters.                                                                                  |
| `INVALID_REQUEST`            | 413    | Request body exceeded 1 MB.                                                                                                                                                                            |
| `BATCH_SIZE_EXCEEDED`        | 400    | More wallets than your batch limit of 50.                                                                                                                                                              |
| `NO_CREDITS`                 | 402    | Match credit balance is exhausted, or the free 30-day allowance is used up. Buy a pack, on either rail below. Only the metered endpoints return it; `/v1/stats` and `/v1/usage` answer at any balance. |
...
Every metered endpoint checks your match balance before it runs. The two free endpoints, [`/v1/stats`](/api-reference/stats) and [`/v1/usage`](/api-reference/usage), skip the refusal and answer at any balance, so a drained key can always read its own meter; that reading is the signal to top up. On a metered endpoint with nothing left, the call returns `402` with one of two `error` strings, depending on whether you are spending the free allowance or a pack:
