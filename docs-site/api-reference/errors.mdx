...
| `ACCOUNT_REQUIRED`           | 403    | A reverse lookup with a key bought with USDC and no account. Those keys resolve addresses; looking up the wallets behind a handle needs an account with an email address. Not worth retrying with the same key.                             |
