...
| `INVALID_REQUEST`            | 413    | Request body exceeded the endpoint’s cap: 1 MB on batch, 4.5 MB on jobs.                                                                                                                                                    |
...
| `NO_CREDITS`                 | 402    | Match credit balance is exhausted, or the free 30-day allowance is used up. Buy a pack, on either rail below. Only the metered endpoints return it; `/v1/stats`, `/v1/usage` and the job status poll answer at any balance. |
...
Every metered endpoint checks your match balance before it runs. The free endpoints, [`/v1/stats`](/api-reference/stats), [`/v1/usage`](/api-reference/usage) and the [job status poll](/api-reference/jobs#polling), skip the refusal and answer at any balance, so a drained key can always read its own meter and collect the results of a job it already paid for; that reading is the signal to top up. On a metered endpoint with nothing left, the call returns `402` with one of two `error` strings, depending on whether you are spending the free allowance or a pack:
