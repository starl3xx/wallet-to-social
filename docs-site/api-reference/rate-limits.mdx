...
`X-Matches-Available` is the balance the request was admitted with, not the balance it left behind: what a call costs is not known until it resolves, so subtract the matches the response reports to know what remains. The header is absent on the legacy unmetered accounts.
