...
### Locked matches on the free allowance

A job on the free allowance can find more matches than the allowance has left,
because submission is bounded in addresses. The job still completes, and you
...
A gated job carries two extra `meta` fields: `matches_delivered` (what was
billed and is open) and `matches_locked` (what is withheld). Unlocking is done
from the app’s results view with pack credits; once unlocked, the same poll
serves every entry in full and the two fields disappear. Jobs run on pack
credits are never gated.
