...
On completion the poll carries the full results, in the [batch](/api-reference/batch) row shape.

<ResponseField name="data.wallets" type="string[]">
  The deduplicated, lowercased submission, in processing order. It rides in the
  same payload as the results because the submission may be sessions old and you
  may no longer hold the list you sent.
</ResponseField>
...
<ResponseField name="meta" type="object">
  `requested`, `found`, `not_found` and `matched`, with the batch arithmetic:
  `matched` is the number the job billed at completion, and it is never more
  than `found`, because ENS, Lens and GitHub identities are free.
</ResponseField>
