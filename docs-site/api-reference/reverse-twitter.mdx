...
Only attested links are returned: a wallet comes back when every source recorded for it is attested evidence (an onchain record, a Farcaster verification, an owner-signed link). A wallet known to carry the handle only through a [correlated](/concepts/data-quality) source is left out, even though a single-wallet lookup still shows it with its label. The total and every page follow the same rule.
