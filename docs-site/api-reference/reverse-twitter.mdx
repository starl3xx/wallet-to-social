...
Finds every wallet attested to an X handle. Costs **1 match credit per wallet returned**, 100 per page. A handle with no wallets attached costs nothing. Separately, each request weighs 2 units against your rate limit.

Only wallets whose recorded sources are all attested evidence (an onchain record, a Farcaster verification, an owner-signed link) are returned. Sources are recorded per wallet, not per handle, so a wallet whose attested link sits beside a [correlated](/concepts/data-quality) source is left out too: we cannot tell which of the two supplied the handle. A single-wallet lookup still shows it, with its labels. The total and every page follow the same rule, and an empty result means no such wallet, not that no address is attested anywhere.

This lookup needs a key that belongs to an account. A key [bought with USDC](/agent-pack) and no account resolves addresses but answers this one with `403` `ACCOUNT_REQUIRED`: turning a handle into the wallets behind it is the direction that can find a person, so somebody has to answer for the search.

This runs the graph backwards, and it answers a question most wallet tooling cannot: which wallets has this account linked to itself? It is a one-time check, for example whether an account holds your token before a partnership, an allowlist or an airdrop. Following what a person holds over time is not what it is for, and the [terms of service](https://walletlink.social/terms#acceptable-use) forbid it.
...
<ResponseField name="data" type="array">
  ...
  apply, plus a flat `quality_score`. The `twitter` object carries
  `self_declared: true` when the wallet’s owner named the handle in their own
  record and nothing else backs it: the owner claims the handle, and the account
  never confirmed it. See [self-declared
  handles](/concepts/data-quality#self-declared-handles).
</ResponseField>
