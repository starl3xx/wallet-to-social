...
counts, the X list export, growing a saved lookup and the API need a pack. The
packs differ only in how many matches they hold; nothing is gated by pack size.
...
Priority score ranks by holdings weighted by Farcaster reach, so the top of a CSV export is the part of your list worth contacting first. It is included with every pack, along with the follower counts it draws on.

## Reaching people directly

Sending Farcaster direct messages to the resolved list from inside the app, without exporting first, is available on legacy Unlimited accounts only. No credit pack includes it. It needs a Farcaster API key of your own, which the app stores only if you ask it to.
