...
unavailable, the import is served from a second index, and behind that a public
backup, so a spent daily ceiling no longer stops the feature. Two networks sit
outside the full arrangement, for different reasons: **BNB Chain** has a
primary index and no backup, so token import there is unavailable until the
ceiling resets, and **HyperEVM** has exactly one index, so a problem with it
pauses token import on that network rather than falling back. NFT collections
are unaffected by a spent ceiling on every network, because they do not resolve
