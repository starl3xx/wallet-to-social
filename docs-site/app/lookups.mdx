...
We cover eight networks: **Ethereum, Base, Robinhood Chain, Arbitrum, Polygon, Optimism, BNB Chain and HyperEVM.** Both NFT collections and token contracts are supported on the first seven. **HyperEVM is NFT collections only**, because nothing indexes token balances on that network in a form we can read; a token import there is refused outright rather than returning a partial list.
...
daily ceiling no longer stops the feature. Two networks sit outside that
arrangement, for different reasons: **BNB Chain** has a primary index and no
backup, so token import there is unavailable until the ceiling resets, and
**HyperEVM** has neither, so it has no token import at all. NFT collections are
unaffected by a spent ceiling on every network, because they do not resolve
through that index.
...
### When a lookup finds more than the allowance covers

Because addresses are bounded at ten times your remaining matches, a
high-hit-rate list on the free allowance can find more matches than the
allowance has left. The lookup still runs in full. The matches your allowance
covered are open in the result; the rest appear as **locked rows**: the wallet
stays, along with anything never billed (an ENS name, a Lens or GitHub
account), and the X handle or Farcaster account is withheld. You are charged
only for the open matches.
