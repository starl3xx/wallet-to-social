...
| — Ethos sweep | Attested wallet-to-X links from an identity platform (`lib/ethos.ts`): the whole dataset enumerated daily in ~80 requests, no key and no metering. Fill-only, quality 60. Its real value is `twitter_user_id`, the permanent X account id, which is the only rot detector in the pipeline. Disagreements go to `handle_conflicts` and are never resolved in code. Daily cron at 06:00 UTC. | |
