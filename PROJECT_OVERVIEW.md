...
| - UD profile harvest        | Attested wallet-to-X links from Unstoppable Domains profiles (`scripts/harvest-ud-profiles.ts`): graph wallets missing an X handle are reverse-resolved to a domain, and the profile's X entry is ingested only where the registry marks it `verified` AND `public` (wallet sign-in plus platform OAuth, both halves owner-established); unverified owner-typed text is counted and skipped. Fill-only through `lib/attested-links.ts`, quality 45, class `attested-social`. Both endpoints keyless with no published terms (provider mid-rebrand, partner program gone), so the harvest is checkpointed in `ingest_state` and budgeted per run. The 2026-09-20 probe measured the address-side arithmetic: 300 most-followed missing-X wallets resolved to 1 domain and 0 verified handles, so the weekly workflow (Sunday 07:45 UTC) is a cheap incremental, and the corpus lives in the domain-side walk (`scripts/harvest-ud-domains.ts`): `NewURI` mints enumerated off the registry contracts (Ethereum CNS + UNS, Polygon UNS, Base UNS), one profile read per domain, current owner via `ownerOf`/Multicall3 on the domain's own chain, same `ud_profile` source id. Probed floors 0.35% (2019 CNS cohort) and 1.85% (2021 Polygon cohort) verified-handle fill. Per-registry checkpoints `ud_domain_enum_*`; twice-daily workflow (02:15/14:15 UTC), 40k reads per run, full backfill roughly seven weeks. Targeted mode mirrors the marketplace enrichment. |                                                                                                                                                                                                                                                    |
