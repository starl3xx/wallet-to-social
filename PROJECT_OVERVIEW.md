...
**Terms at purchase (2026-09-25, STA-47).** A footer link to `/terms` for
everyone, and an explicit agreement where money changes hands. The buy-credits
modal (`components/UpgradeModal.tsx`) has a required, unticked “I agree to the
Terms of Service” box, and `/api/checkout` refuses a request without
`acceptTerms: true` (400 `TERMS_NOT_ACCEPTED`) or with a `termsVersion` other
than the current one (409 `TERMS_VERSION_STALE`), before Stripe is called. The
server stamps `TERMS_VERSION` and its own clock into the session and
PaymentIntent metadata (`termsMetadata`), and all three grant paths (the two
webhook events and the success page's poll) read it back with
`termsAcceptanceFrom` and write it onto the lot as `credit_lots.terms_version`
and `terms_accepted_at`. A paid session with no acceptance (opened before the
checkbox) is still granted, with both columns NULL. The USDC buy cannot add a
required field without breaking paying agents, so its 402 discloses the terms
and the payment is the acceptance (see Onchain rail). `TERMS_VERSION` in
...
│   ├── terms.ts              # TERMS_VERSION, the one terms version; acceptance helpers
...
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `wallet_cache`              | 7-day TTL cache for API results; the daily cleanup deletes expired rows                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | wallet, twitter_handle, farcaster, ens_name, cached_at                                                                                                                                                                                             |
| - Farcaster sweep           | `social_graph` is also bulk-populated by a protocol-wide Farcaster sweep (`lib/farcaster-sweep.ts`): every FID's verified + custody ETH addresses, username, follower count. Daily incremental cron, plus a monthly GitHub Actions run that sweeps **one sixth of the network** (`--slice`, #223), not the whole of it: every FID is re-checked twice a year and each slice cleans up only the revocations in its own FID span. Swept rows are medium quality (45) until a real lookup completes the Twitter side. Revocation cleanup tests the seen set with `NOT EXISTS` and refuses to clear more than 1% of it, and the sweep publishes `posture:farcaster_sweep` on every ending so a failed run says so rather than showing a healthy-looking cursor (2026-09-17/18).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |                                                                                                                                                                                                                                                    |
| - ENS harvest               | Onchain com.twitter/com.github text records (`lib/ens-harvest.ts`): TextChanged log scan → current values via registry→resolver Multicall3 reads. User-attested, `twitter_verified`, quality 50, fill-only. A refused fill earns no source label and no quality bump (2026-09-20): because the handle is written fill-if-empty, a record naming a handle the row already disagrees with is not written, and stamping `ens_onchain` anyway made `isTwitterVerified` read an unearned attestation. The one exception is a record that also fills github, which is a real write. Daily incremental cron from an `ingest_state` checkpoint.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |                                                                                                                                                                                                                                                    |
| - Ethos sweep               | Attested wallet-to-X links from an identity platform, read from CHAIN STATE since 2026-09-20 (`lib/ethos.ts`; write rules shared with the other attested sources in `lib/attested-links.ts`): the platform's terms forbid republishing its API, so the sweep scans the attestation contract's events on Base (service and numeric account id are plain strings onchain), binds profile ids to wallets through the profile contract, and turns ids into handles from the graph first (99.95% free on the backfill) then the resolver within `ETHOS_RESOLVE_CREDITS` (default 900/day); unresolved ids hold the block frontier and share the denial ledger with the clanker sweep. Backfill cross-validated: 87,148 links, 86,235 agreeing with the REST-era rows. Fill-only, quality 45, matching what the live path computes. Its real value is `twitter_user_id`, the permanent X account id, which is the only rot detector in the pipeline. Disagreements go to `handle_conflicts`; the ingest never resolves one (see the conflict resolution row). Daily cron at 06:00 UTC.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |                                                                                                                                                                                                                                                    |
...
| - Basenames harvest         | Onchain `com.twitter` text records on Base names (`lib/basenames.ts`): a `TextChanged` log scan across both L2 resolvers enumerates candidate names, then current values are re-read through registry→resolver Multicall3 reads, and any name past its `nameExpires` is dropped, since an expired name still resolves and its records would otherwise be published as belonging to whoever buys it next. Correlated onchain text, so the same evidence class and the same quality 50 as the ENS harvest, fill-only through `lib/attested-links.ts`. Raw values are validated then rejected, never stripped into shape. One-time backfill CLI, then a daily incremental cron from an `ingest_state` checkpoint at 05:20 UTC.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |                                                                                                                                                                                                                                                    |
| - Creator-profile sweep     | Wallet-to-X links from creator profiles where the X account is attached through a flow the platform records as a dated link event, and wallets are connected to the same account (`lib/zora-profiles.ts`). Two filters decide correctness: only wallets the person brought themselves are read, because the platform also provisions wallets of its own, and an unknown address answers with a wallet-shaped record whose handle field holds a reverse name, which is rejected before it can be ingested as a social handle. Aggregated, quality 35, fill-only: the account half is evidenced by that dated ledger and the wallet half is not evidenced at all, so the pair is corroboration rather than an owner attestation. It supplies no numeric X account id, so it adds reach and no rename detection. A scheduled GitHub Actions workflow (Sunday 07:30 UTC) with a request budget and an `ingest_state` cursor, because an upstream limit of roughly one request a second rules out a 300-second cron.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |                                                                                                                                                                                                                                                    |
| - UD profile harvest        | Attested wallet-to-X links from Unstoppable Domains profiles (`scripts/harvest-ud-profiles.ts`): graph wallets missing an X handle are reverse-resolved to a domain, and the profile's X entry is ingested only where the registry marks it `verified` AND `public` (wallet sign-in plus platform OAuth, both halves owner-established); unverified owner-typed text is counted and skipped. Fill-only through `lib/attested-links.ts`, quality 45, class `attested-social`. Both endpoints keyless with no published terms (provider mid-rebrand, partner program gone), so the harvest is checkpointed in `ingest_state` and budgeted per run. The 2026-09-20 probe measured the address-side arithmetic: 300 most-followed missing-X wallets resolved to 1 domain and 0 verified handles, so the weekly walk is a cheap incremental, and the corpus lives in the domain-side walk (`scripts/harvest-ud-domains.ts`): `NewURI` mints enumerated off the registry contracts (Ethereum CNS + UNS, Polygon UNS, Base UNS), one profile read per domain, current owner via `ownerOf`/Multicall3 on the domain's own chain, same `ud_profile` source id. Probed floors 0.35% (2019 CNS cohort) and 1.85% (2021 Polygon cohort) verified-handle fill. Per-registry checkpoints `ud_domain_enum_*`. **Stopped 2026-09-23:** UD's Terms bar systematic retrieval without written permission, so the launchd agents are unloaded until UD answers the permission request sent 2026-09-24 (Linear STA-13). Targeted mode mirrors the marketplace enrichment. |                                                                                                                                                                                                                                                    |
| - UD onchain validations    | X handles Unstoppable Domains itself verified in 2020 to 2023, read from the chain with no request to UD (`scripts/harvest-ud-validations.ts`, `lib/ud-validations.ts`): UD signed (token id, owner, handle) and wrote the signature as a record, so a domain that changed hands fails the check. Five traps are handled in the verifier (lowercase signer compare, SDK-exact hex bytes, the legacy CNS event, current state via ProxyReader, Polygon-first binding). A one-off: the corpus is frozen. Handles verified for several owners and wallets with several handles are left out, and only handles the X index finds live are ingested, fill-only through `lib/attested-links.ts`, source `ud_twitter_validation`, quality 45, class `attested-social`. 2026-09-24 dry run: 1,020 validated domains, 868 candidate links, 650 live.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |                                                                                                                                                                                                                                                    |
...
| - Conflict resolution       | `lib/conflict-resolution.ts` closes the groups of `handle_conflicts` the evidence decides, two swap rungs since 2026-09-20: ours unreachable on a fresh check (challenger id optional), and ours live but reassigned, resolving to a different account than the graph holds (challenger id required and confirmed). A live ours that cannot be shown wrong is never swapped, whatever the challenger carries (a stolen wallet key plus the attacker's own X account can drive every id-carrying route), so that residue stays on the queue for eyes. Two closure passes mark inert rows without deciding anything: both handles dead, and challenger dead while ours is live (`ours stands`); both reopen their rows if a side comes back. Stale checks are re-run first through `sweepHandles` within `CONFLICT_RECHECK_CREDITS`. Accepting writes `twitter_renamed_from`, the new handle, url, id, verified flag and source, closes the conflict with `accepted-theirs: ours unreachable`, and deletes the `wallet_cache` row. One atomic statement per batch; a second run writes nothing. Both-live conflicts are never swapped. Daily cron 08:40 UTC; `scripts/resolve-handle-conflicts.ts --dry-run` for a manual pass.                                                                                                                                                                                                                                                                                                                        |                                                                                                                                                                                                                                                    |
| `ingest_state`              | Checkpoints for ingest pipelines (name → jsonb value)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | name, value, updated_at                                                                                                                                                                                                                            |
| `seeded_contracts`          | Contracts the daily seed cron has imported holders from (novelty selection: 30-day re-seed window; `resume_state` bookmarks an unfinished holder walk so each slice continues where the last stopped)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | address+chain (pk), name, holders_imported, last_seeded_at                                                                                                                                                                                         |
...
| `handle_conflicts`          | Where two owner-attested sources name different X accounts for one wallet. Recorded by every attested ingest, which became true on 2026-09-20 and was asserted in the same change: until then only the `ingestLinks` callers wrote rows, while `lib/ens-harvest.ts` dropped its disagreements and `lib/farcaster-sweep.ts` overwrote the stored handle without recording one. Resolved only where ours reaches nobody and theirs is live on recent checks (the conflict resolution row above). Measured on 250 real cases, 54% of the time our stored handle no longer resolves, and where both are live, 90% of the time ours belongs to someone who does not claim the wallet, so both-live rows are surfaced and never swapped                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | wallet+platform+their_source (pk), ours, theirs, their_user_id, resolved_at, resolution                                                                                                                                                            |
| `x_accounts`                | X handle reachability, one row per lowercased handle: `live`, `unavailable` or `not_found`, with the display name, follower count and the X platform id. `checked_at` moves on every check, changed or not, and the recheck scheduler reads it to decide what is stale. `last_live_user_id` keeps the last id a handle resolved to and is never cleared, because a suspended handle resolves with no id and clearing it would destroy the row's only durable identifier. `status_changed_at` and `previous_status` (2026-09-18) record when reachability last actually moved and what it moved from; they advance only on a real transition and are NULL for every row written before that date, deliberately un-backfilled, since the first pass observed no transitions and stamping a date would have invented 474k of them. Rechecks begin 2026-10-01, which is the deadline those two columns landed against                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | handle, user_id, last_live_user_id, display_name, followers, status, unavailable_reason, checked_at, status_changed_at, previous_status                                                                                                            |
| `x_list_jobs`               | One row per "turn this result set into an X list" request (2026-09-19). A job rather than a request handler because X allows 300 member additions per 15 minutes, one per request, so a 319-handle list crosses a window boundary; `added_count` is the resume cursor. Holds the authorizing account's sealed X access token for the life of the job and nothing longer: `offline_access` is deliberately not requested, so there is no refresh token anywhere, and `access_token` and `members` are both cleared by whichever of the five end states the job reaches. The token is sealed by `lib/secret-box.ts`, which is what makes the suppression quarantine safe, since an erase copies the whole row into `suppression_quarantine` as jsonb. Carries a `suppression_guard_skip` trigger on `handle`; the members it builds FROM live in a jsonb payload a trigger cannot see, so the worker re-reads the suppression list before each batch. In `READ_ONLY_TABLES`, deliberately not in the backup list, on the `oauth_grants` precedent that restoring a credential resurrects one somebody has finished with A member X refuses every time is stepped over after three attempts at the same cursor position (`stuck_cursor`, `member_attempts`, 2026-09-20), counted `failed` so the list continues; the step-over does not reset `transient_failures`, which is what stops an outage walking the whole list marking everyone unaddable                                                                                                     | user_id, x_user_id, handle, access_token, access_expires_at, code_verifier, state_nonce, list_name, list_description, is_private, members, x_list_id, added_count, skipped_count, failed_count, status, retry_after, stuck_cursor, member_attempts |
| `identity_attestations`     | One row per `/claim`: the owner proving a wallet and an X account together (2026-09-20), and also the pending state for the X round trip, born `awaiting_x` holding a verifier and a nonce. **No access token column**, unlike `x_list_jobs`: that flow spends sixteen minutes adding members, this one reads `/2/users/me` once and drops the credential in the same request. `x_user_id_hmac` is the uniqueness key enforcing one grant per account ever, hashed because it has to keep working after a withdrawal erases the plaintext, which is the entire point of a withdrawal. The wallet stays clear because it is the join key into `social_graph`. Writes reach the index through `ingestLinks` under `owner_attested` (quality 45), FILL-ONLY: where a different handle is already served the disagreement goes to `handle_conflicts` and settles only once ours stops reaching anyone, because overwriting on a signature alone would make a stolen key enough to rewrite an identity. In `READ_ONLY_TABLES`, deliberately not in the backup list (a restore would resurrect an attestation somebody withdrew), and argued into `SUPPRESSION_EXCLUDED_TABLES`: a guard would silently discard the withdrawal, making the one action a person takes to undo their attestation the one action that does nothing. An emailed removal of the wallet or X handle withdraws it too, in `eraseIdentifier` (`withdrawClaimRecords`), exactly as a signed withdrawal does                                                                         | user_id, wallet, signature, x_user_id, x_user_id_hmac, x_handle, consent_version, consent_sha256, status, code_verifier, state_nonce, grant_claimed_at, granted_matches                                                                            |
| `cb_verified_wallets`       | One row per wallet with a live verified-account attestation from the exchange's onchain attester on Base (2026-09-20). A quality signal, never an identity link: no handle, no attested-links involvement. Weekly FULL resync via `cb-verified-sweep.yml`; rows not seen by a complete walk are deleted, so revocations leave. Surfaced only on the admin composition pane (overlap with the graph).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |                                                                                                                                                                                                                                                    |
| `suppressed_identifiers`    | The right-to-removal suppression list (stage 1, 2026-09-02): one row per removed identifier, primary key `(kind, identifier)`, kinds wallet/twitter/farcaster/ens/lens/github. No requester column, deliberately; `lane` records the verification method (values like email, wallet_sig, handle_proof, legal), never the person. Enforced in the database by `BEFORE INSERT OR UPDATE` triggers (`SECURITY DEFINER`) on the seven tables that declare a wallet or handle column (`social_graph`, `wallet_cache`, `x_accounts`, `handle_conflicts`, `social_graph_history`, `wallet_holdings`, `known_agents`, `x_list_jobs`), and pre-flight in `lib/job-processor.ts` so a suppressed wallet never runs the external pipeline. In both `READ_ONLY_TABLES` and the backup list, so a restore cannot un-remove anyone                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | kind, identifier, reason, lane, requested_at, created_at                                                                                                                                                                                           |
| - Removal quarantine        | Before deletion, each removal copies the affected rows to an operator-only quarantine table so a mistaken removal can be undone within 30 days; the cleanup cron purges copies at 30 days. Deliberately in neither `READ_ONLY_TABLES` nor the backup list, because backing it up would extend the stated retention. The dedupe and negative tables (`x_handle_attempts`, the ephemeral sweep seen tables, `clanker_unresolved_ids`) are a documented boundary the suppression system does not touch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |                                                                                                                                                                                                                                                    |
| `lookup_jobs`               | Background job queue. Payloads (the submitted addresses and raw results) are stripped at 30 days by the cleanup cron; the job record itself stays. One pipeline works it, `processJobChunk`, and one invocation at a time: the claim is a conditional UPDATE that sets `leased_until` for 330 seconds and mints `lease_token`, every later write must match that token, and every exit hands the lease back. `slice_attempts` counts claims since the last handback: each killed slice halves the next, and after five the job fails, unbilled. A job whose charge landed is never failed: its finished rows are saved before the charge and it is finished from them. One invocation takes slices, each claimed on its own, for up to 240 seconds (`INVOCATION_BUDGET_MS`) (STA-44)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | status, wallets[], processed_count, partial_results, twitter_found, leased_until, lease_token, slice_attempts                                                                                                                                      |
| `lookup_history`            | Saved lookup sessions. At most one per job: `job_id` is unique and a job saves with ON CONFLICT on `job_id`, updating only the match gate, so a finalize that runs twice saves once and keeps the later gate (STA-44)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | user_id, wallet_count, results (JSONB), input_source                                                                                                                                                                                               |
| `users`                     | User accounts. `tier` is `free` or one of the two legacy values; a pack purchase never changes it. `frozen_at` and `frozen_reason` mark an account frozen after its x402 payer wallet is found on the sanctions list (STA-41): its keys stop validating and it cannot spend                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | email, tier, stripe_customer_id, wallets_used (lifetime record, gates nothing)                                                                                                                                                                     |
| `credit_lots`               | Purchased packs, spent FIFO by expiry. One row per Stripe payment or onchain settlement, recording the terms version agreed to and when (NULL where none was captured). Kept at least 7 years; the purge after that is written and switched off                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `credit_ledger`             | Every match debited, one row per job or API call, and the rolling 30-day free window. Deleted after 7 years unless something live still reads it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | user_id, job_id, matches, wallets_submitted, paid_from, created_at                                                                                                                                                                                 |
| `whitelist`                 | Admin-granted unmetered access; `getUserAccess` reports it as `unlimited`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | email, wallet, note                                                                                                                                                                                                                                |
| `sanctioned_addresses`      | The EVM addresses on OFAC’s SDN list, lowercased (STA-41, 2026-09-25): the list the USDC rail screens payers against. Replaced whole, in one statement, by `/api/cron/sanctions-refresh` every 6 hours, which refuses an empty, older or more than 20% smaller parse. The publish date and last successful refresh are the `sanctions_list` row of `ingest_state`. Rebuildable from OFAC, so not backed up                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | address, sdn_uid, entity, tickers, first_seen_at                                                                                                                                                                                                   |
| `sanctions_screenings`      | The screening record of the USDC rail: a `clear` row per payment that passed verify, and a refusal row (`listed`, `stale`, `missing`) per payer, verdict and hour with an attempt counter and `verify_reached` false. Kept 5 years, then deleted by the daily cleanup; in the nightly backup                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | address, list_publish_date, verdict, verify_reached, attempts, screened_hour, screened_at                                                                                                                                                          |
| `sanctions_freeze_releases` | The (account, payer) pairs an operator released from a sanctions freeze on legal advice (`liftFreeze`, via `scripts/sanctions-lift-freeze.ts`). The freeze leaves them out, so the next refresh does not freeze the account again for them; a payer listed later still does. In the nightly backup                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | user_id, payer, released_at, note                                                                                                                                                                                                                  |
...
| `api_usage`             | Per-request usage tracking; kept 13 months (`API_USAGE_RETENTION_MONTHS`)                                        |
| `rate_limit_buckets`    | Minute, day and month counters per key; deleted 2 days after the period they count ends                          |
...
- `processJobChunk()`: Works one job slice after slice (3,000 wallets each, every slice under its own claim) for up to `INVOCATION_BUDGET_MS` (240 s) per invocation
...
- **`source` is normalized at the boundary** with `asSourceList`, before anything reads it. The field is typed `string[]`, but our own CSV export writes it comma-joined, so a re-uploaded export sends a string back. Untreated it fails twice on the same value: `.some` throws, and spreading it stores a provenance list of single characters.
...
true` and bills nothing. A removal rewrites every stored body naming the
removed identifier and every replay is filtered against the suppression
list; the rows are never deleted, because a deleted key bills the retry
again. Batch rows carry `last_updated` and `stale`
...
under 10 wallets run inline; larger ones are worked straight after the
response (`after()`), slice after slice for up to 240 seconds, and the cron
worker continues anything left each minute.
`GET /v1/jobs/[id]` is zero-cost, scoped strictly to the key's
...
The payment is the acceptance of the terms (2026-09-25, STA-47). The unpaid
402 states `TERMS_URL` and `TERMS_VERSION` in the resource description inside
`PAYMENT-REQUIRED` (all an auto-paying client reads), in a `terms` object in
the JSON body, and in a `Link: <…>; rel="terms-of-service"` header. No new
request field. `grantPackBySettlement` records the version in force when the
payment settled, and the time, on the lot. Zero x402 purchases had been made
when this shipped.
...
**Sanctions screening (STA-41, 2026-09-25).** The buy route screens the
paying wallet against OFAC's SDN list the moment the payer is decoded, before
the lot read, verify and settle (`screenPayer` and `sanctionsRefusal` in
`lib/sanctions.ts`): a listed payer gets 403 `SANCTIONED_PAYER` ("This payment
cannot be accepted."), and a missing list, a list whose last successful refresh
is 7 days old, or a screen that cannot run gets 503 `SCREENING_UNAVAILABLE`.
A refusal is recorded before verify, one row per payer, verdict and hour
with an attempt counter; a `clear` screening is recorded after verify and
before settle (`recordClearScreening`), and a sale it cannot record is
refused. The list is built from
SDN.XML by `/api/cron/sanctions-refresh` every 6 hours: every `0x` + 40-hex
address in an entry, whatever ticker it is filed under (124 in the 2026-09-23
...
`hasPaidAccess` false and every paid-feature gate answering 403
`ACCOUNT_SUSPENDED` with no upsell (`frozenAccountResponse`), new work and
unlocks refused with no upsell, a job or call in flight billed nothing (the
job fails with its results cleared; a charge from an earlier pass is emailed
for a refund decision), and x402 recovery, a top-up with its key
(`isFrozenAccountKey`), a USDC buy into it (after verify) and a signed-in
card checkout all answer the same 403; a card payment that still lands is
granted and emailed to the operator (`lib/account-freeze.ts` lists every
point). Nothing is refunded. An operator lifts a freeze on legal advice with
`scripts/sanctions-lift-freeze.ts` (`liftFreeze`), which records the
released (account, payer) pairs in `sanctions_freeze_releases` so the next
refresh does not freeze the account again for them. The payer that is screened is the
...
batches, because 498,075 of 616,026 rows were past the TTL. `api_usage` is
kept `API_USAGE_RETENTION_MONTHS` (13), `rate_limit_buckets` go
`API_BUCKET_RETENTION_DAYS` (2) after the period they count ends (keyed on the
bucket key, never `created_at`, which would reset a month's quota), and
payment records are kept `PAYMENT_RECORD_RETENTION_YEARS` (7). The ledger
purge after that runs; the `credit_lots` and `users` Stripe-id purge is
written and counted but held off by `PURCHASE_RECORD_PURGE_ENABLED`, because
a lot is also a payment's replay key, the x402 loyalty count and the lifecycle
mail's "has bought" (`lib/retention.ts`). `lifecycle_emails` is kept while the
account exists. Sessions no longer store a user agent. Server log lines mask
emails and wallets: the call sites that print one use `maskEmail` or
`maskWallet`, and `instrumentation.ts` runs every Node log line through
`redact` (`lib/redact.ts`), which is what covers an error object printed
whole. `docs/OPERATIONS.md` has the full cleanup table.
...
Stage 1 of the right to removal (2026-09-02) laddered the removal section:
removal is by email and executed by hand, suppression is permanent because
the suppression list blocks re-collection, a 30-day quarantine copy exists
solely so a mistaken removal can be undone, and saved customer lookups have
the identifier removed. The no-proof sentence, the jurisdiction-blind stance
and the 30-day SLA all stand; exports a customer already downloaded stay out
of reach, as the page has always said. The retention table gained two rows
...
Stage 1 of the right-to-removal system (2026-09-02) adds an operator removal
surface behind the same admin auth (`lib/admin-auth.ts`): execute a removal
(insert and commit the suppression row first, then copy-and-delete each
affected row into quarantine in one atomic statement per table, then amend
saved results, in that order) and un-suppress within the 30-day
quarantine window. The runbook, the uniform reply script included, is in
`docs/OPERATIONS.md`.
...
# Optional
INNGEST_EVENT_KEY=...                    # Retired route only (registers nothing since STA-44); goes with it
INNGEST_SIGNING_KEY=...
