...
| - Conflict resolution       | `lib/conflict-resolution.ts` closes the groups of `handle_conflicts` the evidence decides, three swap rungs since 2026-09-20: ours unreachable on a fresh check (challenger id optional); ours live but reassigned, resolving to a different account than the graph holds (challenger id required and confirmed); and an id-anchored live-confirmed challenger against a bare string (no condition on ours). Where both sides are live and id-consistent nothing acts: that residue stays on the queue for eyes. Two closure passes mark inert rows without deciding anything: both handles dead, and challenger dead while ours is live (`ours stands`); both reopen their rows if a side comes back. Stale checks are re-run first through `sweepHandles` within `CONFLICT_RECHECK_CREDITS`. Accepting writes `twitter_renamed_from`, the new handle, url, id, verified flag and source, closes the conflict with `accepted-theirs: ours unreachable`, and deletes the `wallet_cache` row. One atomic statement per batch; a second run writes nothing. Both-live conflicts are never swapped. Daily cron 08:40 UTC; `scripts/resolve-handle-conflicts.ts --dry-run` for a manual pass.                                                                                                                                                                                                                                                                             |                                                                                                                                                                                                                                                    |
